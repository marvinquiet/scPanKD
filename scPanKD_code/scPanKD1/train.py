...
## import my package
from models.batch_MLP import batch_MLP
...
def train_batch_MLP(args, model_config):
    ...
    batch_mlp = batch_MLP(dims=model_config['MLP_DIMS'], 
            input_dim=x_train.shape[1],
            n_batch=len(set(batch_info)),
            n_classes=y_train.shape[1]).to(model_config['device'])
    if model_config['optimizer'] == 'adam':
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, batch_mlp.parameters()), lr=model_config['learning_rate'])
    if model_config['optimizer'] == 'adamW':
        optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, batch_mlp.parameters()), lr=model_config['learning_rate'])
    ...
    dataset = torch.utils.data.TensorDataset(torch.tensor(x_train, dtype=torch.float32, device=model_config['device']),
                                             torch.tensor(batch_info, dtype=torch.long, device=model_config['device']),
    ...
    for epoch in tqdm(range(model_config['max_epochs'])):
        ...
        for batch_x, batch_b, batch_y in dataloader:
            optimizer.zero_grad()
            outputs, _ = batch_mlp(batch_x, batch_b)
            loss_cls = criterion(outputs, torch.argmax(batch_y, dim=1))
            # add batch adversarial loss
            batch_logits = batch_mlp(batch_x, batch_b, reverse=True)
    ...
    torch.save(batch_mlp.state_dict(), model_save_dir+os.sep+'model.pth')

    # visualize predicted features for training data
    _, features = batch_mlp.predict(torch.tensor(x_train, dtype=torch.float32, device=model_config['device']))
