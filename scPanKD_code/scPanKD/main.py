...
def parse_args():
    ...
    # ===========================
    # Parser for a batch of training gene score matrices to a model
    # --batch_dir
    # -i, --input
    # --batch_info
    # -m, --metadata
    ...
    # --num_features
    # ============================
    train_batch_parser = subparsers.add_parser('train_batch', help='Train a Cellcano model in a batch mode.')
    train_batch_parser.add_argument('--batch_dir', dest='batch_dir',
            type=str,
            help="Filepath for loading batch information")
    train_batch_parser.add_argument('-i', '--input', dest='input', 
            type=str, nargs='+',
            help="A COO matrix prefix or a csv input.")
    train_batch_parser.add_argument('--batch_info', dest='batch_info',
            type=int, nargs='+',
            help='Batch information')
    train_batch_parser.add_argument('-m', '--metadata', dest='metadata', 
            type=str, 
            help="The annotation dataframe with row as barcodes or cell ID and columns as celltype. Notice: please make sure that your cell type indicator be 'celltype'.")
    train_batch_parser.add_argument('--model', dest='model',
            type=str, default='MLP_batch',
            help="Batch-Model used to train the data.")
    train_batch_parser.add_argument('-o', '--output_dir', dest='output_dir', 
            type=str, default="output",
            help="Output directory.")
    train_batch_parser.add_argument('--prefix', dest='prefix', 
            type=str, default='train_',
            help="Output prefix.")
    train_batch_parser.add_argument('--fs', dest='fs', 
            help="Feature selection methods.", 
            choices=["F-test", "noFS", "seurat"], type=str, default="F-test")
    train_batch_parser.add_argument('--num_features', dest='num_features', 
            help="Feature selection methods.", type=int, default=3000)
...
def main():
    ...
    ## === training process 
    if "train" == args.cmd_choice or "train_batch" == args.cmd_choice:
        ...
        # train - MLP batch version
        if "train_batch" == args.cmd_choice:
            train.train_batch_MLP(args)
    ## === prediction process
