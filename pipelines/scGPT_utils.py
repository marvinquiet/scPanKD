...
def run_scGPT_finetune(config, train_adata, test_adata, scGPT_result_dir,
    ...
    save_dir = Path(f"{scGPT_result_dir}/save/dev_{dataset_name}-{time.strftime('%b%d-%H-%M')}/")
    save_dir.mkdir(parents=True, exist_ok=True)
    print(f"save to {save_dir}")
    scg.utils.add_file_handler(logger, save_dir / "run.log")

    # --- preprocess the data ---
    train_adata.obs["batch_id"]  = train_adata.obs["str_batch"] = "0"
    test_adata.obs["batch_id"] = test_adata.obs["str_batch"] = "1"
    data_is_raw = False
    filter_gene_by_counts = False
    test_adata_raw = test_adata.copy()
    adata = train_adata.concatenate(test_adata, batch_key="str_batch") ## concatenate the train and test data
    # make the batch category column
    batch_id_labels = adata.obs["str_batch"].astype("category").cat.codes.values
    adata.obs["batch_id"] = batch_id_labels
    celltype_id_labels = adata.obs[train_celltype_col].astype("category").cat.codes.values
    celltypes = adata.obs[train_celltype_col].unique()
    num_types = len(np.unique(celltype_id_labels))
    id2type = dict(enumerate(adata.obs[train_celltype_col].astype("category").cat.categories))
    ...
    adata_train = adata[adata.obs["str_batch"] == "0"]
    ...
    optimizer_D, optimizer_E = None, None
    if ADV:
    ...
    for epoch in range(1, epochs + 1):
        ...
        if config.do_train:
            train(
                ...
                adv_D_delay_epochs=adv_D_delay_epochs,
                adv_E_delay_epochs=adv_E_delay_epochs
            )
    ...
    # --- run UMAP
    sc.pp.neighbors(test_adata_raw, use_rep="X", n_neighbors=15, n_pcs=30)
    sc.tl.umap(test_adata_raw, random_state=42)
...
def init_wandb_config(scGPT_model_dir, WANDB_MODE="offline"):
...
def prepare_data(sort_seq_batch=False,
                 ...
                 valid_celltype_labels=None) -> Tuple[Dict[str, torch.Tensor]]:
...
def train(model: nn.Module, loader: DataLoader,
          ...
          dab_weight=None,
          adv_D_delay_epochs=None,
          adv_E_delay_epochs=None
          ) -> None:
    """
